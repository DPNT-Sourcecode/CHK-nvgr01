...
const calculateBestDiscount = (total, countOfEachSku, discounts, sku, discountedAppliedForSku) => {
  ...
  for (let i = discounts.length - 1; i >= 0; i--) {
    ...
    if (currentSKUCount >= currentDiscount["quantity"]) {
      ...
      const resultTotal = calculatePriceAfterDiscount(newTotal, countOfEachSku, currentDiscount, sku, numberOfDiscounts, discountedAppliedForSku);
      newTotal = resultTotal;
...
const checkForBulkDiscount = (countOfBulkDiscountSkus) => {
  ...
  for (let sku in countOfBulkDiscountSkus) {
    if (countOfBulkDiscountSkus[sku] > 0) {
      totalItems += countOfBulkDiscountSkus[sku];
    }
  }
  if (totalItems >= 3) {
    result = true;
...
const calculatePriceAfterDiscount = (total, countOfEachSku, discount, SKU, numberOfDiscounts, discountedAppliedForSku) => {
  let newTotal = total;
  console.log("starting Total", newTotal);
  ...
  if (discount.hasOwnProperty("bulk") && !discountedAppliedForSku[SKU]) {
    ...
    for (let i = 0; i < bulkDiscount["skus"].length; i++) {
      countOfBulkDiscountSkus[bulkDiscount["skus"][i]] = countOfEachSku[bulkDiscount["skus"][i]] ? countOfEachSku[bulkDiscount["skus"][i]] : 0;
    ...
    while (checkForBulkDiscount(countOfBulkDiscountSkus)) {
      ...
      for (let i = 0; i < bulkDiscount["skus"].length; i++) {
        ...
        console.log("sku", sku, "counter", counter);
        if (countOfBulkDiscountSkus[sku] > 0) {
          let numberOfItemsToDeduct = 1;
          // minimum to deduct is 1;
          // maximum to deduct 3 - counter

          if (countOfBulkDiscountSkus[sku] === 1) {
            numberOfItemsToDeduct = 1;
          } else if (countOfBulkDiscountSkus[sku] === 2) {
            numberOfItemsToDeduct = 2;
          } else {
            numberOfItemsToDeduct = 3;
          }
          if (3 - counter <= countOfBulkDiscountSkus[sku]) {
            numberOfItemsToDeduct = 3 - counter;
          } 
          console.log("numberOfItemsToDeduct", numberOfItemsToDeduct);
          countOfBulkDiscountSkus[sku] -= numberOfItemsToDeduct;
          countOfBulkDiscountSkusApplied[sku] += numberOfItemsToDeduct;
          counter += numberOfItemsToDeduct;
        }
      }
      bulkDiscountsApplied += 1;
    }

    console.log("bulkDiscountsApplied", bulkDiscountsApplied);
