...
const calculatePriceAfterDiscount = (total, countOfEachSku, discount, SKU, numberOfDiscounts, discountedAppliedForSku) => {
    ...
    if (discount.hasOwnProperty("free")) {
        ...
        if (numberOfDiscounts > 0 && !discountedAppliedForSku[freeItem["sku"]]) {
            ...
            if (SKU === freeItem["sku"]) {
                // If the free item is the same as the SKU then we need to deduct one from the number of free items
                numberOfFreeItems = Math.floor(countOfEachSku[freeItem["sku"]] / (freeItem["quantity"] + 1));
                console.log("Number of free items: ", numberOfFreeItems);
            }
