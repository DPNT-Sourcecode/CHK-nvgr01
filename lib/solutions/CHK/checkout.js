...
module.exports = function (skus) {
    ...
    for (const sku in discountTable) {
        ...
        for (let i = 0; i < discounts.length; i++) {
            const totalAfterDiscount = calculatePriceAfterDiscount(total, countOfEachSku, discounts[i], sku);
            console.log("sku", sku);
            console.log("discounts[i]", discounts[i]);
            console.log("totalAfterDiscount", totalAfterDiscount);
            if (totalAfterDiscount < totalAfterDiscount) {
