...
const calculatePriceAfterDiscount = (total, countOfEachSku, discount, SKU, discountedAppliedForSku) => {
    ...
    if (discount.hasOwnProperty("free")) {
        ...
        if (numberOfDiscounts > 0) {
            if (discountedAppliedForSku[freeItem["sku"]]) {
                // remove the discount for the free item if it has been applied then apply the discount again and compute the new total
                const freeItemNumberOfDiscounts = getNumberOfDiscount
