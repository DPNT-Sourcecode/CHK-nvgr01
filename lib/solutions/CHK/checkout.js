...
module.exports = function (skus) {
    ...
    for (const discount in discountTable) {
        const discounts = discountTable[discount];
        const [minDiscount, maxDiscount] = getMinMaxDiscount(discounts);
        if (countOfEachSku[discount] < minDiscount["quantity"]) {
            continue;
        }
        const numberOfDiscounts = getNumberOfDiscount(countOfEachSku[discount], maxDiscount["quantity"]);
        total -= priceTable[discount] * maxDiscount["quantity"] * numberOfDiscounts;
        total += maxDiscount["discount"] * numberOfDiscounts;
    }
