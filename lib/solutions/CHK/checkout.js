...
const calculateBestDiscount = (count, discounts) => {
    let minimumCost = count * priceTable[discounts[0]["sku"]];
    for (let discount of discounts) {
        let remainig = count;
        let totalCost = 0;
        while (remainig >= discount["quantity"]) {
            remainig -= discount["quantity"];
            totalCost += discount["discount"];
        }
        totalCost += remainig * priceTable[discounts[0]["sku"]];
...
const calculatePriceAfterDiscount = (total, countOfEachSku, discount, SKU, discountedAppliedForSku) => {
    let newTotal = total;
...
module.exports = function (skus) {
    ...
    //         const currentTotalDiscount = calculatePriceAfterDiscount(total, countOfEachSku, discounts[i], sku, discountedAppliedForSku);
    ...
    for (const sku in discountTable) {
        ...
        let bestCost = calculateBestDiscount(countOfEachSku[sku], discounts);
        total += (bestCost - countOfEachSku[sku] * priceTable[sku]);
    }
