...
const calculateBestDiscount = (total, countOfEachSku, discounts, sku, discountedAppliedForSku) => {
    ...
    let newTotal = total;
    let currentDiscount = discounts[0];
    let currentDiscountIndex = discounts.length - 1;
    let currentSKUCount = countOfEachSku[sku];

    while (currentDiscountIndex >= 0) {
        currentDiscount = discounts[currentDiscountIndex];
        if (currentSKUCount >= currentDiscount["quantity"]) {
            newTotal = calculatePriceAfterDiscount(newTotal, countOfEachSku, currentDiscount, sku, discountedAppliedForSku);
            currentSKUCount = countOfEachSku[sku];
            currentDiscountIndex = discounts.length - 1;
        }
    }
