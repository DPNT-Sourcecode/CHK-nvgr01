...
const getNumberOfDiscountForA = (countOfA) => {
    const numberOfDiscounts = divideAndRoundDown(countOfA, 3);
...
module.exports = function (skus) {
    ...
    const numberOfDiscountsForA = getNumberOfDiscountForA(countOfEachSku["A"]);
    if (numberOfDiscountsForA >= 1) {
        // Apply the discount for A
        total -= priceTable["A"] * 3;
