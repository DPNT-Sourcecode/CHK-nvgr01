...
module.exports = function (skus) {
    ...
    for (let i = 0; i < allDiscountOrders.length; i++) {
        const discountOrder = allDiscountOrders[i];
        let newTotal = total;
        let newCountOfEachSku = JSON.parse(JSON.stringify(countOfEachSku));
        let newDiscountedAppliedForSku = JSON.parse(JSON.stringify(discountedAppliedForSku));

        // Check for any discounts
        for (const sku in discountOrder) {
            const discounts = discountOrder[sku];
            const [minDiscount,] = getMinMaxDiscount(discounts);
            if (newCountOfEachSku[sku] < minDiscount["quantity"]) {
                continue;
            }
            newTotal = calculateBestDiscount(newTotal, newCountOfEachSku, discounts, sku, newDiscountedAppliedForSku);
        }

        if (newTotal < total) {
            total = newTotal;
            countOfEachSku = newCountOfEachSku;
            discountedAppliedForSku = newDiscountedAppliedForSku;
        }
    }
